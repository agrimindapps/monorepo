...
class DiagnosticosRepositoryImpl implements IDiagnosticosRepository {
  final DiagnosticoCoreRepository _coreRepository;

  const DiagnosticosRepositoryImpl(this._coreRepository);
  ...
  Future<Either<Failure, List<DiagnosticoEntity>>> getAll({
    ...
    try {
      final diagnosticosHive = await _coreRepository.getAllAsync();
  ...
  Future<Either<Failure, DiagnosticoEntity?>> getById(String id) async {
    try {
      final diagnosticoHive = await _coreRepository.getByIdAsync(id);
  ...
  Future<Either<Failure, List<DiagnosticoEntity>>> getByDefensivo(String idDefensivo) async {
    try {
      final diagnosticosHive = await _coreRepository.findByDefensivo(idDefensivo);
  ...
  Future<Either<Failure, List<DiagnosticoEntity>>> getByCultura(String idCultura) async {
    try {
      final diagnosticosHive = await _coreRepository.findByCultura(idCultura);
      final entities = diagnosticosHive
          .map<DiagnosticoEntity>((hive) => DiagnosticoMapper.fromHive(hive))
          .toList();
  ...
  Future<Either<Failure, List<DiagnosticoEntity>>> getByPraga(String idPraga) async {
    try {
      final diagnosticosHive = await _coreRepository.findByPraga(idPraga);
      final entities = diagnosticosHive
          .map<DiagnosticoEntity>((hive) => DiagnosticoMapper.fromHive(hive))
          .toList();
  ...
  Future<Either<Failure, List<DiagnosticoEntity>>> getByTriplaCombinacao({
    ...
    try {
      final diagnosticosHive = await _coreRepository.findByMultipleCriteria(
  ...
  Future<Either<Failure, DiagnosticosStats>> getStatistics() async {
    try {
      final stats = await _coreRepository.getDiagnosticoStats();
