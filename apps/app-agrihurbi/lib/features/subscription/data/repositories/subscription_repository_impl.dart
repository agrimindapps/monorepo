...
class SubscriptionRepositoryImpl implements SubscriptionRepository {
  ...
  Future<Either<Failure, UserSubscription>> upgradePlan(String userId, String newPlanId) async {
    try {
      ...
      final upgradedSubscription = await remoteDataSource.upgradePlan(userId, newPlanId);
  ...
  Stream<Either<Failure, UserSubscription?>> watchSubscription(String userId) {
    try {
      return remoteDataSource.watchSubscription(userId).map((subscription) {
        if (subscription != null) {
        ...
        return Left<Failure, UserSubscription?>(
          ServerFailure(message: 'Erro ao observar assinatura: ${error.toString()}'),
        );
      });
    } catch (e) {
      return Stream.value(
        Left(ServerFailure(message: 'Erro ao observar assinatura: ${e.toString()}')),
      );
    }
  }
}
