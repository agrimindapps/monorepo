import 'package:core/core.dart' hide AuthStatus;
...
/// Página de exclusão de conta para Petiveti
/// Padronizada com app-gasometer
...
class _AccountDeletionPageState extends ConsumerState<AccountDeletionPage> {
  ...
  Future<void> _handleAccountDeletion() async {
    ...
    if (authState.status == AuthStatus.authenticated && !authState.isAnonymous) {
    ...
    try {
      final result = await authNotifier.deleteAccount(password: currentPassword);
      final errorMessage = ref.read(authProvider).error;
      if (!result || errorMessage != null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(errorMessage ?? 'Erro ao excluir conta'),
              backgroundColor: Colors.red,
