...
/// com SnackBar de undo (5 segundos por padrão).
///
/// Exemplo de uso:
/// ```dart
/// SwipeToDeleteWrapper(
///   itemKey: 'animal_${animal.id}',
///   deletedMessage: 'Pet excluído',
///   onDelete: () => notifier.removeOptimistic(animal.id),
///   onRestore: () => notifier.restore(animal.id),
...
/// ```dart
/// AnimalCard(animal: animal).withSwipeToDelete(
///   itemKey: 'animal_${animal.id}',
///   deletedMessage: 'Pet excluído',
///   onDelete: () => notifier.removeOptimistic(animal.id),
///   onRestore: () => notifier.restore(animal.id),
...
class SwipeToDeleteWrapper extends StatelessWidget {
  ...
  /// Deve executar a exclusão otimista (remove da UI imediatamente)
  final Future<void> Function() onDelete;

  /// Callback executado quando o usuário clica em "Desfazer"
  /// Deve restaurar o item na UI
  ...
  /// Callback opcional para confirmar a exclusão antes de executar
  /// Retorna null ou false para cancelar, true para confirmar
  final Future<bool?> Function(DismissDirection)? confirmDismiss;

  /// Se o swipe está habilitado
  /// Útil para desabilitar em modos de edição/seleção
  final bool enabled;
  ...
  Future<void> _handleDismiss(BuildContext context) async {
    // Executa a exclusão otimista (remove da UI)
    await onDelete();
...
/// Exemplo:
/// ```dart
/// MyListItem().withSwipeToDelete(
///   itemKey: 'item_123',
///   deletedMessage: 'Item excluído',
///   onDelete: () => deleteItem(),
