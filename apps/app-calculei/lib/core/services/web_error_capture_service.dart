...
class WebErrorCaptureService {
  ...
  /// Calculadora atual (se aplicável)
  String? _currentCalculatorId;
  String? _currentCalculatorName;
  ...
  void captureNetworkError({
    ...
    final errorMessage = 'Network Error: $statusCode - ${message ?? 'Unknown'} for $url';
    _captureError(
      error: errorMessage,
  ...
  void captureTimeoutError({
    ...
    final errorMessage = 'Timeout: $operation${timeout != null ? ' after ${timeout.inSeconds}s' : ''}';
    _captureError(
      error: errorMessage,
  ...
  /// Define a calculadora atual
  void setCurrentCalculator({String? id, String? name}) {
    _currentCalculatorId = id;
    _currentCalculatorName = name;
  }
  ...
  void _captureError({
    ...
    final errorLog = ErrorLogEntity(
      ...
      url: _currentUrl,
      calculatorId: _currentCalculatorId,
    ...
    debugPrint('[WebErrorCaptureService] Error captured: ${errorType.name} - ${message.substring(0, message.length.clamp(0, 100))}');
  ...
  String _generateErrorHash(String message, String? stackTrace) {
    final content = '$message|${stackTrace?.split('\n').take(5).join('\n') ?? ''}';
  ...
  Future<void> _incrementOccurrences(String hash) async {
    try {
      ...
      debugPrint('[WebErrorCaptureService] Incremented occurrences for hash: ${hash.substring(0, 8)}');
    } catch (e) {
  ...
  Future<void> _flushQueue() async {
    ...
    debugPrint('[WebErrorCaptureService] Uploading ${errorsToUpload.length} errors...');
