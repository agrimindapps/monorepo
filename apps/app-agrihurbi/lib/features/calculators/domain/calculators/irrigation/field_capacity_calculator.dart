...
class FieldCapacityCalculator extends CalculatorEntity {
  const FieldCapacityCalculator()
      : super(
          ...
          description: 'Calcula a capacidade de retenção de água do solo e parâmetros para manejo da irrigação',
  ...
  CalculationResult calculate(Map<String, dynamic> inputs) {
    try {
      ...
      final double sandContent = double.parse(inputs['sand_content'].toString());
  ...
  String _classifySoilPhysicalQuality(
    ...
    if (totalPorosity > 0.55 && macroporosity > 0.15 && availableWater > 0.15) {
      return 'Excelente';
    } else if (totalPorosity > 0.50 && macroporosity > 0.10 && availableWater > 0.12) {
      return 'Boa';
    } else if (totalPorosity > 0.45 && macroporosity > 0.08 && availableWater > 0.10) {
      return 'Regular';
    } else {
      return 'Ruim';
    }
