...
class AppDataManager implements IAppDataManager {
  ...
  Future<Either<Exception, void>> initialize() async {
    ...
    try {
      developer.log('Iniciando inicialização do sistema de dados com controle de versão...', name: 'AppDataManager');
      ...
      developer.log('Executando controle automático de versão...', name: 'AppDataManager');
      
      final versionControlResult = await _versionControlService.executeVersionControl(
        onProgress: (stage, message, progress) {
          developer.log('[$stage] $message (${(progress * 100).toInt()}%)', name: 'AppDataManager');
        },
      );

      if (versionControlResult.isLeft()) {
        final error = versionControlResult.fold((e) => e.toString(), (r) => '');
        developer.log('Erro no controle de versão: $error', name: 'AppDataManager');
        return Left(Exception('Falha no controle de versão: $error'));
      }

      final result = versionControlResult.fold((l) => null, (r) => r)!;
      developer.log('Controle de versão executado: ${result.toString()}', name: 'AppDataManager');

      // 6. Verifica se dados estão carregados
      final isDataReady = await _dataService.isDataLoaded();
      if (!isDataReady) {
        return Left(Exception('Dados não foram carregados corretamente após controle de versão'));
      }

      _isInitialized = true;
      _versionControlService.markAsInitialized();
      
      developer.log('Sistema de dados inicializado com sucesso via controle automático de versão', 
        name: 'AppDataManager');
  ...
  Future<void> _createServices() async {
    try {
      ...
      developer.log('Serviços criados com sucesso (incluindo controle de versão)', name: 'AppDataManager');
  ...
  Future<Either<Exception, void>> forceReloadData() async {
    ...
    try {
      developer.log('Forçando recarregamento via controle de versão...', name: 'AppDataManager');
      ...
      return result.fold(
        (error) => Left(error),
        (versionResult) => versionResult.success 
          ? const Right(null)
          : Left(Exception('Recarregamento falhou: ${versionResult.message}')),
      );
      
    } catch (e) {
      developer.log('Erro ao forçar recarregamento: $e', name: 'AppDataManager');
      return Left(Exception('Falha no recarregamento forçado: ${e.toString()}'));
    }
  }
  ...
  Future<Map<String, dynamic>> getDataStats() async {
    ...
    try {
      ...
      developer.log('Erro ao obter estatísticas combinadas: $e', name: 'AppDataManager');
      return {
        'error': e.toString(),
