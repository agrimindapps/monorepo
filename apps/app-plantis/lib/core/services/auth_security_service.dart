...
class AuthSecurityService {
  ...
  static AuthSecurityService get instance => _instance ??= AuthSecurityService._();
  ...
  Future<bool> isAccountLockedOut(String userIdentifier) async {
    try {
      ...
      final lockoutTimestamp = prefs.getInt('${_lockoutTimestampKey}_$userIdentifier');
      
      if (lockoutTimestamp == null) return false;
      
      final lockoutTime = DateTime.fromMillisecondsSinceEpoch(lockoutTimestamp);
      final isStillLocked = DateTime.now().difference(lockoutTime) < _lockoutDuration;
  ...
  Future<void> recordFailedLoginAttempt(String userIdentifier) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final currentAttempts = prefs.getInt('${_failedAttemptsKey}_$userIdentifier') ?? 0;
      final newAttempts = currentAttempts + 1;
      
      await prefs.setInt('${_failedAttemptsKey}_$userIdentifier', newAttempts);
      await prefs.setInt('${_lastLoginAttemptKey}_$userIdentifier', 
          DateTime.now().millisecondsSinceEpoch);
      
      debugPrint('⚠️ Failed login attempt $newAttempts/$_maxFailedAttempts for $userIdentifier');
  ...
  Future<void> _lockAccount(String userIdentifier) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setInt('${_lockoutTimestampKey}_$userIdentifier', 
          DateTime.now().millisecondsSinceEpoch);
      
      debugPrint('🔒 Account locked for $userIdentifier due to too many failed attempts');
    } catch (e) {
  ...
  bool _isCommonPassword(String password) {
    final commonPasswords = [
      ...
      'login', 'password1', '123123', 'qwerty123', 'user',
    ];
