...
class MarketsListPage extends ConsumerStatefulWidget {
  const MarketsListPage({super.key});

  @override
  ConsumerState<MarketsListPage> createState() => _MarketsListPageState();
}

class _MarketsListPageState extends ConsumerState<MarketsListPage>
          ...
          if (provider.currentFilter.hasActiveFilters)
            Container(
              ...
              child: Wrap(
                spacing: 8,
                children:
                    provider.currentFilter.activeFiltersDescription
                        .map(
                          (desc) => Chip(
                            label: Text(
                              desc,
                    ...
                    : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return MarketCard(
                          ...
                          onFavoriteToggle:
                              () => provider.toggleFavorite(market.id),
                        );
                      },
                    ),
          ),
  ...
  Widget _buildFavoritesTab(MarketProvider provider) {
    return RefreshIndicator(
      ...
      child:
          provider.isLoadingFavorites && provider.favoriteMarkets.isEmpty
              ? const _LoadingList()
