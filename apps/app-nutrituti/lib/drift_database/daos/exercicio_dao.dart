...
class ExercicioDao extends DatabaseAccessor<NutitutiDatabase> with _$ExercicioDaoMixin {
  ExercicioDao(NutitutiDatabase db) : super(db);
  ...
  Future<Exercicio?> getExercicioById(String id) {
    return (select(exercicios)..where((tbl) => tbl.id.equals(id))).getSingleOrNull();
  }

  /// Get exercicios by date range
  Future<List<Exercicio>> getExerciciosByDateRange(DateTime start, DateTime end) {
    return (select(exercicios)
      ..where((tbl) => 
  ...
  Future<List<Exercicio>> getPendingSyncExercicios() {
    return (select(exercicios)
      ..where((tbl) => tbl.isPending.equals(true) & tbl.isSynced.equals(false))
  ...
  Future<int> updateExercicio(String id, ExerciciosCompanion exercicio) {
    return (update(exercicios)..where((tbl) => tbl.id.equals(id)))
        .write(exercicio.copyWith(updatedAt: Value(DateTime.now())));
  }

  /// Mark as synced
  Future<int> markAsSynced(String id) {
    return (update(exercicios)..where((tbl) => tbl.id.equals(id)))
        .write(ExerciciosCompanion(
  ...
  Future<double> getTotalCaloriasByDateRange(DateTime start, DateTime end) async {
    ...
    return exerciciosList.fold<double>(0.0, (sum, e) => sum + e.caloriasQueimadas);
  }
