...
abstract class VaccineRemoteDataSource {
  ...
  Future<VaccineModel> scheduleVaccineReminder(
    String vaccineId,
...
class VaccineRemoteDataSourceImpl implements VaccineRemoteDataSource {
  ...
  Future<List<VaccineModel>> getVaccinesNeedingReminders() async {
    try {
      ...
      throw ServerException(
        message: 'Erro ao buscar vacinas para lembrete: $e',
      );
    }
  }
    ...
    try {
      ...
      if (updatedVaccine == null) {
        throw const ServerException(
          message: 'Vacina não encontrada após agendamento de lembrete',
        );
      }
    ...
    try {
      ...
      return vaccines.where((vaccine) {
        return vaccine.name.toLowerCase().contains(queryLower) ||
            ...
            (vaccine.manufacturer?.toLowerCase().contains(queryLower) ??
                false) ||
  ...
  Future<List<VaccineModel>> getVaccinesByVeterinarian(
    ...
    try {
      ...
      throw ServerException(
        message: 'Erro ao buscar vacinas por veterinário: $e',
      );
    }
  }
  ...
  Future<List<VaccineModel>> getVaccinesByManufacturer(
    ...
    try {
      ...
      throw ServerException(
        message: 'Erro ao buscar vacinas por fabricante: $e',
      );
    }
  }

  @override
  Future<List<VaccineModel>> addMultipleVaccines(
  ...
  Future<void> markVaccinesAsCompleted(List<String> vaccineIds) async {
    try {
      ...
      throw ServerException(
        message: 'Erro ao marcar vacinas como concluídas: $e',
      );
    }
  }
  ...
  Future<DateTime?> getLastSyncTime() async {
    try {
      ...
      throw ServerException(
        message: 'Erro ao obter tempo de sincronização: $e',
      );
    }
  }
  ...
  Future<void> updateLastSyncTime() async {
    try {
      await _firestore
          ...
          .collection('metadata')
          .doc('vaccines_sync')
          .set({'lastSync': DateTime.now().millisecondsSinceEpoch});
    } catch (e) {
      throw ServerException(
        message: 'Erro ao atualizar tempo de sincronização: $e',
      );
    }
  }

  @override
  Future<List<VaccineModel>> getVaccinesModifiedAfter(
    DateTime timestamp,
  ) async {
    try {
      final snapshot = await _vaccinesCollection
