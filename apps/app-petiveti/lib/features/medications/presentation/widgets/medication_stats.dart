...
class MedicationStats extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final completedCount = medications
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          if (typeStats.isEmpty) const Text('Nenhum medicamento cadastrado'),
        ],
      ),
    );
  }
  ...
  Widget _buildStatusChart(BuildContext context, List<Medication> medications) {
    ...
    for (final medication in medications) {
      statusCounts[medication.status] =
          (statusCounts[medication.status] ?? 0) + 1;
    }
  ...
  Widget _buildTypeStatRow(
    ...
    return Row(
      children: [
        Expanded(
          flex: 3,
          child: Text(type.displayName, style: theme.textTheme.bodyMedium),
        ),
        Expanded(
          flex: 2,
