...
class CalculateEmergencyReserveUseCase {
  Future<Either<Failure, EmergencyReserveCalculation>> call(
      CalculateEmergencyReserveParams params) async {
  ...
  ValidationFailure? _validate(CalculateEmergencyReserveParams params) {
    if (params.monthlyExpenses <= 0) {
      return const ValidationFailure(
          'Despesas mensais devem ser maiores que zero');
    }

    if (params.monthlyExpenses > 1000000) {
      return const ValidationFailure(
          'Despesas mensais não podem exceder R\$ 1.000.000,00');
    }
    ...
    if (params.extraExpenses > 1000000) {
      return const ValidationFailure(
          'Despesas extras não podem exceder R\$ 1.000.000,00');
    }
    ...
    if (params.desiredMonths > 120) {
      return const ValidationFailure(
          'Número de meses não pode exceder 120 (10 anos)');
    }

    if (params.monthlySavings < 0) {
      return const ValidationFailure(
          'Valor poupado mensalmente não pode ser negativo');
    }

    if (params.monthlySavings > 1000000) {
      return const ValidationFailure(
          'Valor poupado mensalmente não pode exceder R\$ 1.000.000,00');
    }

    return null;
  }

  EmergencyReserveCalculation _performCalculation(
      CalculateEmergencyReserveParams params) {
  ...
  Map<String, String> _determineCategory(int months) {
    if (months < 3) {
      return {
        'category': 'Mínima',
        'description':
            'Cobertura apenas para emergências imediatas e muito básicas.',
      };
    } else if (months >= 3 && months < 6) {
      return {
        'category': 'Básica',
        'description':
            'Nível recomendado para pessoas com emprego estável e sem dependentes.',
      };
    } else if (months >= 6 && months < 12) {
      return {
        'category': 'Confortável',
        'description':
            'Ideal para quem tem família ou trabalha como autônomo/freelancer.',
      };
    } else {
      return {
        'category': 'Robusta',
        'description':
            'Reserva sólida para longos períodos ou grandes imprevistos.',
      };
    }
  }
}
