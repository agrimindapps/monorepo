...
class NotificationAnalyticsHelper {
  ...
  static const int _maxStoredEvents =
      10000; // Limit stored events to prevent memory issues
  ...
  Future<void> trackEvent(NotificationEvent event) async {
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '📊 Tracked notification event: ${event.type} for notification ${event.notificationId}',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<NotificationAnalytics> getAnalytics(
    ...
    try {
      ...
      final filteredEvents =
          events.where((event) {
            final isInRange =
                event.timestamp.isAfter(dateRange.startDate) &&
                event.timestamp.isBefore(dateRange.endDate);

            final matchesPlugin =
                pluginId == null || event.pluginId == pluginId;
    ...
    try {
      ...
      final userEvents =
          events.where((event) {
            ...
            final isUserEvent =
                event.metadata.containsKey('userId') &&
    ...
    try {
      ...
      for (final event in events) {
        if (event.timestamp.isAfter(dateRange.startDate) &&
            event.timestamp.isBefore(dateRange.endDate)) {
          notificationEvents
              .putIfAbsent(event.notificationId, () => [])
              .add(event);
        }
      }
      ...
      for (final notificationId in notificationEvents.keys) {
        ...
        notificationEventList.sort(
          (a, b) => a.timestamp.compareTo(b.timestamp),
        );
        final scheduledEvent = notificationEventList.firstWhereOrNull(
          (e) => e.type == NotificationEventType.scheduled,
        );
        final deliveredEvent = notificationEventList.firstWhereOrNull(
          (e) => e.type == NotificationEventType.delivered,
        );
        final clickedEvent = notificationEventList.firstWhereOrNull(
          (e) => e.type == NotificationEventType.clicked,
        );
  ...
  Future<void> clearOldData(Duration olderThan) async {
    try {
      ...
      if (kDebugMode) {
        debugPrint(
          '🧹 Cleared ${events.length - recentEvents.length} old notification events',
        );
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<List<NotificationEvent>> _getStoredEvents(
  ...
  Future<void> _saveEvents(
    ...
    final eventsJson =
        events.map((event) => jsonEncode(_eventToJson(event))).toList();
  ...
  NotificationAnalytics _calculateAnalytics(
    ...
    final scheduledEvents =
        events.where((e) => e.type == NotificationEventType.scheduled).toList();
    final deliveredEvents =
        events.where((e) => e.type == NotificationEventType.delivered).toList();
    final clickedEvents =
        events.where((e) => e.type == NotificationEventType.clicked).toList();
    final dismissedEvents =
        events.where((e) => e.type == NotificationEventType.dismissed).toList();
    ...
    for (final pluginId in pluginEvents.keys) {
      ...
      final pluginDelivered =
          pluginEventList
              .where((e) => e.type == NotificationEventType.delivered)
              .length;
      final pluginClicked =
          pluginEventList
              .where((e) => e.type == NotificationEventType.clicked)
              .length;

      performanceByPlugin[pluginId] =
          pluginDelivered > 0 ? pluginClicked / pluginDelivered : 0.0;
    }
    ...
    while (date.isBefore(endDate) || date.isAtSameMomentAs(endDate)) {
      ...
      final dayEvents =
          events
              .where(
                (event) =>
                    event.timestamp.isAfter(date) &&
                    event.timestamp.isBefore(nextDate),
              )
              .toList();

      final scheduled =
          dayEvents
              .where((e) => e.type == NotificationEventType.scheduled)
              .length;
      final delivered =
          dayEvents
              .where((e) => e.type == NotificationEventType.delivered)
  ...
  UserEngagementMetrics _calculateUserEngagement(
    ...
    final totalClicked = clickedEvents.length;
    final engagementRate =
        totalReceived > 0 ? totalClicked / totalReceived : 0.0;
    ...
    final averageResponseTime =
        responseCount > 0
            ? Duration(
              milliseconds: totalResponseTime.inMilliseconds ~/ responseCount,
            )
            : Duration.zero;
    final engagementByPlugin = <String, int>{};
    for (final event in clickedEvents) {
      if (event.pluginId != null) {
        engagementByPlugin[event.pluginId!] =
            (engagementByPlugin[event.pluginId!] ?? 0) + 1;
      }
    }
    ...
    final preferredHours =
        hourCounts.entries
            .where(
              (entry) => entry.value >= 2,
            ) // At least 2 clicks in that hour
