...
class MigrationCLI {
    ...
    try {
      ...
      final result = {
        ...
        'summary': {
          ...
          'migrated_apps': appStatuses.values.where((s) => s.migrationCompleted).length,
          'pending_migration': appStatuses.values.where((s) => !s.migrationCompleted).length,
        }
      };
      ...
      for (final appName in appStatuses.keys) {
        ...
        if (appStatus != null) {
          (result['apps'] as Map<String, dynamic>)[appName] = {
            ...
            'success_rate': '${(appStatus.successRate * 100).toStringAsFixed(1)}%',
      ...
      developer.log(
        'Error executing status command: $e',
        name: 'MigrationCLI',
      );
      return {
  ...
  Future<Map<String, dynamic>> commandCheck(String appName) async {
    ...
    try {
      ...
      final result = {
        ...
        'recommended_migration_time': report.recommendedMigrationTime.toIso8601String(),
        ...
        'current_statistics': {
          ...
          'success_rate': '${(report.currentStatistics.successRate * 100).toStringAsFixed(1)}%',
  ...
  Future<Map<String, dynamic>> commandMigrate(
    ...
    try {
      ...
      final response = {
        ...
        'steps': result.steps.map((MigrationStep step) => {
          'name': step.name,
        ...
        'new_service_stats': {
          ...
          'success_rate': '${(result.newServiceStatistics.successRate * 100).toStringAsFixed(1)}%',
        },
  ...
  Future<Map<String, dynamic>> commandRollback(String appName) async {
    ...
    try {
      ...
      final rollbackResult = await AppMigrationHelper.instance.rollbackMigration(appName);
  ...
  Future<Map<String, dynamic>> commandEnableFlags() async {
    try {
      developer.log(
        'Executing enable-flags command',
        name: 'MigrationCLI',
      );
  ...
  Future<Map<String, dynamic>> commandDisableFlags() async {
    try {
      developer.log(
        'Executing disable-flags command',
        name: 'MigrationCLI',
      );
      ...
      return {
        ...
        'success': true,
        'message': 'Global feature flags disabled - all apps using legacy UnifiedSyncManager',
        'enabled_flags': {
          'use_new_sync_orchestrator': flags.useNewSyncOrchestrator,
          'enabled_apps': flags.enabledApps,
        },
  ...
  Map<String, dynamic> commandHelp() {
    return {
      'migration_cli_help': {
        'version': '1.0.0',
        'description': 'CLI para migração do UnifiedSyncManager para nova arquitetura SOLID',
        'commands': {
          'status': {
          ...
          'check <app>': {
            'description': 'Verifica compatibilidade de migração para um app específico',
            'usage': 'MigrationCLI.instance.commandCheck("gasometer")',
            'parameters': ['app: Nome do app (gasometer, plantis, receituagro, petiveti)'],
            'returns': 'Relatório de compatibilidade com riscos e recomendações',
          },
          'migrate <app>': {
            'description': 'Executa migração para nova arquitetura',
            'usage': 'MigrationCLI.instance.commandMigrate("gasometer", dryRun: false)',
            'parameters': [
              'app: Nome do app',
