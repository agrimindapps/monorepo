...
/// ComentariosController refatorado usando DisposableMixin para cleanup adequado
/// 
/// Esta versão elimina vazamentos de memória através de:
/// - Tracking automático de Timer de debounce
/// - Cleanup adequado de TextEditingController
/// - Uso de CompositeSubscription para múltiplas subscriptions
/// - Leak detection integrado em debug mode
class ComentariosControllerEnhanced extends GetxController with DisposableMixin {
  final ComentariosService _service = Get.find<ComentariosService>();
  
  // Controllers são automaticamente registrados para cleanup
  late final TextEditingController searchController;
  late final CompositeSubscription _subscriptions;
  
  // Timer é registrado automaticamente
  ...
  void _initializeController() {
    // Registra automaticamente os controllers para cleanup
    searchController = registerTextController(TextEditingController());
    _subscriptions = createCompositeSubscription();
    
    // Adiciona listener que será automaticamente removido
  ...
  void _onSearchChanged() {
    ...
    // Cancela timer anterior se existir - será automaticamente limpo no dispose
    _debounceTimer?.cancel();
    _debounceTimer = registerTimer(Timer(const Duration(milliseconds: 300), () {
  ...
  // DisposableMixin automaticamente limpa todos os recursos registrados
  // Não precisamos mais implementar onClose() manualmente!
}