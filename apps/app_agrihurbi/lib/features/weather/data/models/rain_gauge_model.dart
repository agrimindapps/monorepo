...
class RainGaugeModel extends Equatable {
  ...
  const RainGaugeModel({
    ...
    required this.dailyAccumulation,
    required this.weeklyAccumulation,
    required this.monthlyAccumulation,
    required this.yearlyAccumulation,
    required this.maxRainfallRate,
    required this.avgRainfallRate,
    required this.lastMeasurement,
    required this.nextMeasurement,
    required this.measurementInterval,
    required this.latitude,
    required this.longitude,
    required this.heightAboveGround,
  ...
  factory RainGaugeModel.fromJson(Map<String, dynamic> json) {
    return RainGaugeModel(
      ...
      currentRainfall: (json['current_rainfall']?.toDouble()) ?? 0.0,
      dailyAccumulation: (json['daily_accumulation']?.toDouble()) ?? 0.0,
      weeklyAccumulation: (json['weekly_accumulation']?.toDouble()) ?? 0.0,
      monthlyAccumulation: (json['monthly_accumulation']?.toDouble()) ?? 0.0,
      yearlyAccumulation: (json['yearly_accumulation']?.toDouble()) ?? 0.0,
      maxRainfallRate: (json['max_rainfall_rate']?.toDouble()) ?? 0.0,
      avgRainfallRate: (json['avg_rainfall_rate']?.toDouble()) ?? 0.0,
      ...
      measurementInterval: json['measurement_interval']?.toInt() ?? 60,
      latitude: (json['latitude']?.toDouble()) ?? 0.0,
      longitude: (json['longitude']?.toDouble()) ?? 0.0,
      heightAboveGround: (json['height_above_ground']?.toDouble()) ?? 0.0,
      calibrationFactor: (json['calibration_factor']?.toDouble()) ?? 1.0,
      status: json['status']?.toString() ?? 'inactive',
      batteryLevel: json['battery_level']?.toDouble(),
      signalStrength: json['signal_strength']?.toDouble(),
      temperature: json['temperature']?.toDouble(),
      dataQuality: (json['data_quality']?.toDouble()) ?? 0.0,
  ...
  factory RainGaugeModel.fromIoTData(
    ...
    final deviceInfo = deviceData['device_info'] ?? {};
    final measurements = deviceData['measurements'] ?? {};
    ...
    return RainGaugeModel(
      ...
      currentRainfall: (measurements['current_rainfall']?.toDouble()) ?? 0.0,
      dailyAccumulation: (measurements['daily_total']?.toDouble()) ?? 0.0,
      weeklyAccumulation: (measurements['weekly_total']?.toDouble()) ?? 0.0,
      monthlyAccumulation: (measurements['monthly_total']?.toDouble()) ?? 0.0,
      yearlyAccumulation: (measurements['yearly_total']?.toDouble()) ?? 0.0,
      maxRainfallRate: (measurements['max_rate']?.toDouble()) ?? 0.0,
      avgRainfallRate: (measurements['avg_rate']?.toDouble()) ?? 0.0,
      ...
      measurementInterval: measurements['interval_minutes']?.toInt() ?? 60,
      latitude: (deviceData['location']?['latitude']?.toDouble()) ?? 0.0,
      longitude: (deviceData['location']?['longitude']?.toDouble()) ?? 0.0,
      heightAboveGround: (deviceInfo['height_m']?.toDouble()) ?? 1.5,
      calibrationFactor: (deviceInfo['calibration']?.toDouble()) ?? 1.0,
      status: status['status']?.toString() ?? 'unknown',
      batteryLevel: status['battery_percent']?.toDouble(),
      signalStrength: status['signal_strength']?.toDouble(),
      temperature: measurements['ambient_temp']?.toDouble(),
      dataQuality: (status['data_quality']?.toDouble()) ?? 1.0,
  ...
  RainGaugeModel copyWith({
    ...
    return RainGaugeModel(
      ...
      dailyAccumulation: dailyAccumulation ?? this.dailyAccumulation,
      weeklyAccumulation: weeklyAccumulation ?? this.weeklyAccumulation,
      monthlyAccumulation: monthlyAccumulation ?? this.monthlyAccumulation,
      yearlyAccumulation: yearlyAccumulation ?? this.yearlyAccumulation,
      maxRainfallRate: maxRainfallRate ?? this.maxRainfallRate,
      avgRainfallRate: avgRainfallRate ?? this.avgRainfallRate,
      lastMeasurement: lastMeasurement ?? this.lastMeasurement,
      nextMeasurement: nextMeasurement ?? this.nextMeasurement,
