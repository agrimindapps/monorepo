...
/// Dialog showing all Sudoku achievements organized by category
class SudokuAchievementsDialog extends ConsumerWidget {
  const SudokuAchievementsDialog({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final achievementsAsync = ref.watch(sudokuAchievementsProvider);
    final stats = ref.watch(sudokuAchievementStatsProvider);
  ...
  Widget _buildHeader(BuildContext context, SudokuAchievementStats stats) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildSummary(SudokuAchievementStats stats) {
    return Container(
  ...
  Widget _buildTabs(
      BuildContext context, List<SudokuAchievement> achievements) {
    return DefaultTabController(
      length: SudokuAchievementCategory.values.length,
      child: Column(
        children: [
          TabBar(
            isScrollable: true,
            ...
            tabs: SudokuAchievementCategory.values.map((cat) {
              final count = achievements
                  .where((a) => a.definition.category == cat && a.isUnlocked)
                  .length;
              final total =
                  SudokuAchievementDefinitions.getByCategory(cat).length;
              return Tab(
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
          ...
          Expanded(
            child: TabBarView(
              children: SudokuAchievementCategory.values.map((cat) {
  ...
  Widget _buildCategoryList(List<SudokuAchievement> achievements) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return SudokuAchievementCard(achievement: achievements[index]);
      },
    );
  }
...
/// Card showing a single Sudoku achievement
class SudokuAchievementCard extends StatelessWidget {
  final SudokuAchievement achievement;

  const SudokuAchievementCard({super.key, required this.achievement});
  ...
  Color _getUnlockedColor(SudokuAchievementRarity rarity) {
...
/// Dialog shown when Sudoku achievement is unlocked
class SudokuAchievementUnlockedDialog extends StatefulWidget {
  final SudokuAchievementDefinition achievement;

  const SudokuAchievementUnlockedDialog({
    super.key,
    required this.achievement,
  });

  @override
  State<SudokuAchievementUnlockedDialog> createState() =>
      _SudokuAchievementUnlockedDialogState();
}

class _SudokuAchievementUnlockedDialogState
    extends State<SudokuAchievementUnlockedDialog>
...
class SudokuAchievementsUnlockedOverlay extends StatefulWidget {
  final List<SudokuAchievementDefinition> achievements;
  final VoidCallback onComplete;

  const SudokuAchievementsUnlockedOverlay({
    super.key,
  ...
  @override
  State<SudokuAchievementsUnlockedOverlay> createState() =>
      _SudokuAchievementsUnlockedOverlayState();
}

class _SudokuAchievementsUnlockedOverlayState
    extends State<SudokuAchievementsUnlockedOverlay> {
  int _currentIndex = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: SudokuAchievementUnlockedDialog(
...
class SudokuAchievementBadge extends StatelessWidget {
  final SudokuAchievement achievement;
  final double size;

  const SudokuAchievementBadge({
    super.key,
    required this.achievement,
    this.size = 40,
  });
...
class SudokuAchievementsProgressIndicator extends ConsumerWidget {
  const SudokuAchievementsProgressIndicator({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final completionPercent =
        ref.watch(sudokuAchievementsCompletionPercentProvider);
    final unlockedCount =
        ref.watch(sudokuUnlockedAchievementsCountProvider);
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              '$unlockedCount/${SudokuAchievementDefinitions.totalCount}',
              style: const TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
              ),
