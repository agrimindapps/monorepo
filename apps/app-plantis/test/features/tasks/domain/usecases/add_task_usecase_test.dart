...
void main() {
  ...
  group('AddTaskUseCase', () {
    test(
      ...
      () async {
        // Arrange
        final testTask = TestFixtures.createTestTask(title: 'Water Plant');

        when(
          () => mockTasksRepository.addTask(any()),
    ...
    test('should handle multiple task additions sequentially', () async {
      ...
      when(
        () => mockTasksRepository.addTask(task1),
      ).thenAnswer((_) async => Right(task1));
      when(
        () => mockTasksRepository.addTask(task2),
