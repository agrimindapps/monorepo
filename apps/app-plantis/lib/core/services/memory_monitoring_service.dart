...
import '../../features/plants/data/datasources/local/plants_search_service.dart';
import '../../features/plants/presentation/widgets/optimized_plant_image_widget.dart';
...
class MemoryMonitoringService {
  ...
  static MemoryMonitoringService get instance => _instance ??= MemoryMonitoringService._();
  ...
  void startMonitoring() {
    ...
    _isMonitoring = true;
    _monitoringTimer = Timer.periodic(_monitoringInterval, (_) => _takeSnapshot());
  ...
  Future<MemorySnapshot> _getCurrentMemorySnapshot() async {
    ...
    final imageCacheStats = <String, dynamic>{}; // OptimizedPlantImageWidget.getCacheStats();
    final searchCacheStats = <String, dynamic>{}; // PlantsSearchService.instance.getCacheStats();
  ...
  void _checkMemoryThresholds(MemorySnapshot snapshot) {
    ...
    if (usedMemory > _criticalThresholdMB) {
      debugPrint('🚨 CRITICAL: Memory usage is ${usedMemory}MB (>${_criticalThresholdMB}MB)');
      ...
      debugPrint('⚠️ WARNING: Memory usage is ${usedMemory}MB (>${_warningThresholdMB}MB)');
    }
  }
  ...
  void _logSnapshot(MemorySnapshot snapshot) {
    ...
    buffer.writeln('  Image Cache: ${snapshot.imageCacheStats['cachedImages']} images (${snapshot.imageCacheStats['totalSizeMB']}MB)');
    buffer.writeln('  Search Cache: ${snapshot.searchCacheStats['cacheSize']} queries');
  ...
  MemoryReport getMemoryReport() {
    ...
    if (usedMemories.isNotEmpty) {
      averageMemory = usedMemories.reduce((a, b) => a + b) / usedMemories.length;
    ...
    if (current.usedMemoryMB != null && current.usedMemoryMB! > _warningThresholdMB) {
      recommendations.add('Memory usage is high (${current.usedMemoryMB}MB)');
    }
    
    if (trend == MemoryTrend.increasing) {
      recommendations.add('Memory usage is increasing over time - check for leaks');
    }
    
    final totalImageCacheMB = double.tryParse(current.imageCacheStats['totalSizeMB'] ?? '0') ?? 0;
    if (totalImageCacheMB > 20) {
      recommendations.add('Image cache is large (${totalImageCacheMB}MB) - consider clearing');
    }
