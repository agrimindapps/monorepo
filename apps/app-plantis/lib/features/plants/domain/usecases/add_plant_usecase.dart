...
class AddPlantUseCase implements UseCase<Plant, AddPlantParams> {
  ...
  Future<void> _generateInitialTasks(Plant plant) async {
    ...
    try {
      ...
      result.fold(
        (failure) => print('Warning: Falha ao gerar tarefas iniciais: ${failure.message}'),
        (tasks) => print('Info: ${tasks.length} tarefas iniciais geradas para planta ${plant.name}'),
      );
    } catch (e) {
      // Log do erro (não bloqueia o fluxo principal)
      print('Warning: Erro inesperado ao gerar tarefas iniciais: $e');
    }
  }
