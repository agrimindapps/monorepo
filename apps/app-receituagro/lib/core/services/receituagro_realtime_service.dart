...
/// Serviço para gerenciar sincronização em tempo real do ReceitaAgro
/// com otimizações de performance e bateria
///
/// Baseado no PlantisRealtimeService (Gold Standard 10/10)
class ReceitaAgroRealtimeService with WidgetsBindingObserver {
  static final ReceitaAgroRealtimeService _instance =
      ReceitaAgroRealtimeService._internal();
  static ReceitaAgroRealtimeService get instance => _instance;
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      developer.log(
        'Erro ao inicializar ReceitaAgroRealtimeService: $e',
        name: 'RealtimeService',
      );
    }
  }
  ...
  Future<void> forceSync() async {
    try {
      await UnifiedSyncManager.instance.forceSyncApp('receituagro');
  ...
  void _handleAppResumed() async {
    ...
    await forceSync(); // Sincroniza ao voltar para foreground
  ...
  void _handleAppPaused() async {
    // Mantém realtime por 5 minutos após ir para background
    // Útil para multitasking rápido
  ...
  bool _shouldUseRealtime() {
    // Ativa se app está em foreground
    if (_currentLifecycleState == AppLifecycleState.resumed) {
      return true;
    }

    // Mantém ativo se está em background MAS timer ainda está ativo (< 5min)
  ...
  Future<void> dispose() async {
    try {
      ...
      developer.log('ReceitaAgroRealtimeService disposed', name: 'RealtimeService');
    } catch (e) {
      developer.log(
        'Erro ao fazer dispose do ReceitaAgroRealtimeService: $e',
        name: 'RealtimeService',
      );
    }
  }
