...
extension FutureResultExtensions<T> on Future<Result<T>> {
  ...
  Future<Result<T>> mapErrorResult(AppError Function(AppError error) mapper) async {
    final result = await this;
    return result.mapError(mapper);
  }

  /// Executa uma operação em cadeia
  Future<Result<R>> thenResult<R>(Future<Result<R>> Function(T data) operation) async {
    final result = await this;
    return await result.then(operation);
  }

  /// Adiciona tratamento de erro
  Future<Result<T>> catchError(AppError Function(dynamic error, StackTrace stackTrace) errorHandler) async {
    try {
      return await this;
