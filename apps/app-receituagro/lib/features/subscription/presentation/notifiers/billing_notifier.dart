...
class BillingNotifier extends _$BillingNotifier {
  ...
  Future<BillingState> build() async {
    _errorService = SubscriptionErrorMessageService();
  ...
  Future<void> retryFailedBilling({required String issueId}) async {
    ...
    final issue = currentState.issues.firstWhere(
      (i) => i.id == issueId,
    ...
    try {
      ...
      final newIssues = currentState.issues
  ...
  Future<void> updatePaymentMethod({required String paymentMethodToken}) async {
    ...
    try {
      ...
      for (var issue in currentState.issues) {
  ...
  Future<void> dismissWarning({
    ...
    try {
      ...
      // Remover da lista
      final newIssues = currentState.issues.where((i) => i.id != issueId).toList();
      state = AsyncValue.data(currentState.copyWith(issues: newIssues));
    } catch (error) {
      state = AsyncValue.data(
        currentState.copyWith(
  ...
  void addBillingIssue(BillingIssueEntity issue) {
    ...
    // Evitar duplicatas
    final exists = currentState.issues.any(
      (i) => i.billingIssueCode == issue.billingIssueCode,
    );
    if (exists) return;

    final newIssues = [...currentState.issues, issue];
    state = AsyncValue.data(currentState.copyWith(issues: newIssues));
  }
  ...
  Duration? getTimeUntilNextRetry() {
    ...
    final issuesWithRetry = currentState.issues
  ...
  void clearState() {
    state = AsyncValue.data(BillingState.initial());
  }
}
