...
class SettingsLocalDataSource {
  ...
  Future<void> saveNotificationSettings(NotificationSettings settings) async {
    final currentSettings =
        await loadSettings() ?? SettingsData.defaultSettings();
  ...
  Future<NotificationSettings> _migrateLegacyNotificationSettings() async {
    try {
      ...
      final overdueNotifications =
          _prefs.getBool('notifications_overdue') ?? true;
      final dailySummary =
          _prefs.getBool('notifications_daily_summary') ?? true;
      final reminderMinutes =
          _prefs.getInt('notifications_reminder_minutes') ?? 60;
      final dailySummaryHour =
      ...
      for (final taskType in taskTypes) {
        taskTypeSettings[taskType] =
            _prefs.getBool('notifications_task_type_$taskType') ?? true;
      }
