...
class PlantsSearchService {
  ...
  static PlantsSearchService get instance => _instance ??= PlantsSearchService._();
  
  PlantsSearchService._();
  
  Database? _database;
  final Map<String, List<PlantaModel>> _searchCache = {};
  final Map<String, Set<String>> _wordIndex = {};
  final Map<String, PlantaModel> _plantsIndex = {};
  ...
  /// Update search index with new plant data (legacy model)
  Future<void> updateSearchIndex(List<PlantaModel> plants) async {
    await initialize();
    if (_database == null) return;
  ...
  Future<void> updateSearchIndexFromPlants(List<Plant> plants) async {
    ...
    for (final plant in plants) {
      ...
      // Build memory index - convert to PlantaModel for compatibility
      final legacyPlant = PlantaModel(
        id: plant.id,
        nome: plant.name,
        especie: plant.species,
        observacoes: plant.notes,
      );
      _plantsIndex[plant.id] = legacyPlant;
      _buildWordIndex(legacyPlant);
    }
    
    await batch.commit(noResult: true);
  }
  
  void _buildWordIndex(PlantaModel plant) {
    final words = <String>{};
  ...
  Future<List<PlantaModel>> searchPlants(String query) async {
  ...
  Future<List<PlantaModel>> _searchMemoryIndex(String query) async {
  ...
  Future<List<PlantaModel>> _searchFTS(String query) async {
    await initialize();
    if (_database == null) return [];
  ...
  Future<List<PlantaModel>> _searchHybrid(String query) async {
    ...
    final mergedResults = <String, PlantaModel>{};
  ...
  List<PlantaModel>? _getCachedResult(String query) {
  ...
  void _cacheResult(String query, List<PlantaModel> results) {
  ...
  Future<List<PlantaModel>> searchWithDebounce(
    String query,
    Duration delay,
  ) async {
    final completer = Completer<List<PlantaModel>>();
