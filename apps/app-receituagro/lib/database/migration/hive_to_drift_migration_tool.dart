...
import '../../core/data/models/favorito_item_hive.dart';
import '../../core/data/models/comentario_hive.dart';
...
class HiveToDriftMigrationTool {
  ...
  Future<MigrationResult> migrate() async {
    developer.log('🔄 Iniciando migração Hive → Drift...', name: 'HiveToDriftMigration');
    ...
    try {
      ...
      developer.log('✅ Migração concluída em ${result.durationSeconds}s', name: 'HiveToDriftMigration');
  ...
  ///
  /// TODO: Implementar carregamento real dos JSON assets
  /// Por enquanto, apenas verifica se já existem dados
  Future<void> _populateStaticData() async {
    developer.log('📦 Verificando dados estáticos...', name: 'HiveToDriftMigration.staticData');

    // Verificar se já existem culturas
    final culturasCount = await (_db.selectOnly(_db.culturas)
    ...
    if (count > 0) {
      developer.log('ℹ️ Dados estáticos já populados (${count} culturas)', name: 'HiveToDriftMigration.staticData');
      return;
    }

    developer.log(
      '⚠️ Dados estáticos não implementados. Migration dos diagnósticos pode falhar devido a FKs não resolvidas.',
  ...
  Future<int> _migrateDiagnosticos() async {
    developer.log('📦 Migrando diagnosticos...', name: 'HiveToDriftMigration.diagnosticos');
    ...
    final hiveItems = boxResult.fold(
      (failure) {
        developer.log('⚠️ Falha ao abrir box diagnosticos: $failure', name: 'HiveToDriftMigration.diagnosticos');
    ...
    if (hiveItems.isEmpty) {
      developer.log('  ⚠️  Nenhum diagnostico encontrado no Hive', name: 'HiveToDriftMigration.diagnosticos');
      return 0;
    }

    developer.log('  📊 ${hiveItems.length} diagnosticos encontrados no Hive', name: 'HiveToDriftMigration.diagnosticos');
        ...
        try {
          ...
          await _db.into(_db.diagnosticos).insert(
            DiagnosticosCompanion.insert(
              firebaseId: Value(hiveItem.objectId.isNotEmpty ? hiveItem.objectId : null),
  ...
  Future<int> _migrateFavoritos() async {
    developer.log('📦 Migrando favoritos...', name: 'HiveToDriftMigration.favoritos');
    ...
    final hiveItems = boxResult.fold(
      (failure) {
        developer.log('⚠️ Falha ao abrir box favoritos: $failure', name: 'HiveToDriftMigration.favoritos');
    ...
    if (hiveItems.isEmpty) {
      developer.log('  ⚠️  Nenhum favorito encontrado no Hive', name: 'HiveToDriftMigration.favoritos');
      return 0;
    }

    developer.log('  📊 ${hiveItems.length} favoritos encontrados no Hive', name: 'HiveToDriftMigration.favoritos');
        ...
        try {
          await _db.into(_db.favoritos).insert(
            FavoritosCompanion.insert(
              firebaseId: Value(hiveItem.sync_objectId.isNotEmpty ? hiveItem.sync_objectId : null),
              userId: '', // TODO: Resolver userId real
  ...
  Future<int> _migrateComentarios() async {
    developer.log('📦 Migrando comentarios...', name: 'HiveToDriftMigration.comentarios');
    ...
    final hiveItems = boxResult.fold(
      (failure) {
        developer.log('⚠️ Falha ao abrir box comentarios: $failure', name: 'HiveToDriftMigration.comentarios');
    ...
    if (hiveItems.isEmpty) {
      developer.log('  ⚠️  Nenhum comentario encontrado no Hive', name: 'HiveToDriftMigration.comentarios');
      return 0;
    }

    developer.log('  📊 ${hiveItems.length} comentarios encontrados no Hive', name: 'HiveToDriftMigration.comentarios');
        ...
        try {
          ...
          await _db.into(_db.comentarios).insert(
            ComentariosCompanion.insert(
              firebaseId: Value(
                hiveItem.sync_objectId != null && hiveItem.sync_objectId!.isNotEmpty
              ...
              createdAt: Value(
                hiveItem.sync_createdAt != null
                    ? DateTime.fromMillisecondsSinceEpoch(hiveItem.sync_createdAt!)
                    : DateTime.now(),
              ),
              updatedAt: Value(
                hiveItem.sync_updatedAt != null
                    ? DateTime.fromMillisecondsSinceEpoch(hiveItem.sync_updatedAt!)
                    : null,
              ),
              itemId: hiveItem.pkIdentificador, // pkIdentificador = ID do item comentado
