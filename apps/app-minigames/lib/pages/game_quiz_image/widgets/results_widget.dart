...
class _ResultsWidgetState extends State<ResultsWidget>
  ...
  void initState() {
    ...
    // Se o score for bom, inicia a animação
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        // Efeito de celebração personalizado para substituir o confetti
        if (percentage >= 70)
          AnimatedBuilder(
...
// Implementação personalizada para o efeito de celebração
class CelebrationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < particleCount; i++) {
      ...
      // Movimentação das partículas com base no progresso da animação
      final x = baseX + math.sin((progress * 2 * math.pi) + i) * 50;
      final y =
          baseY - progress * (size.height * 0.3) * (1 + random.nextDouble());

      // Tamanho variável para efeito de pulsação
      final particleSize =
