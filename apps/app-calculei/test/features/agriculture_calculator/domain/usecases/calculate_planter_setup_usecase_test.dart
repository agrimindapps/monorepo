...
void main() {
  ...
  group('CalculatePlanterSetupUseCase', () {
    group('Success Cases', () {
      test(
        'should calculate planter setup successfully for Soja with valid data',
        () async {
          // Arrange
      ...
      test(
        'should use correct thousand seed weight for each crop type',
        () async {
          final cropWeights = {
            'Soja': 180.0,
            'Milho': 350.0,
          ...
          for (final entry in cropWeights.entries) {
            ...
            final params = CalculatePlanterSetupParams(
              cropType: entry.key,
              targetPopulation: entry.key == 'Milho'
                  ? 65000
                  : entry.key == 'Girassol'
                  ? 50000
                  : entry.key == 'Algodão'
            ...
            result.fold(
              (failure) => fail(
                'Should not return failure for ${entry.key}: ${failure.message}',
              ),
              (calculation) {
                expect(
      ...
      test('should assign correct wheel turns based on disc holes', () async {
        ...
        for (final entry in discToTurns.entries) {
          ...
          result.fold(
            (failure) =>
                fail('Should not return failure for ${entry.key} holes'),
            (calculation) {
              expect(
                calculation.wheelTurns,
                entry.value,
                reason:
                    'Wheel turns for ${entry.key} holes should be ${entry.value}',
              );
            },
          );
        }
    ...
    group('Validation - Crop Type', () {
      test(
        'should return ValidationFailure when crop type is invalid',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
    ...
    group('Validation - Population', () {
      ...
      test(
        'should return ValidationFailure when population is below minimum for crop',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
      ...
      test(
        'should return ValidationFailure when population is above maximum for crop',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Milho',
    ...
    group('Validation - Row Spacing', () {
      test(
        'should return ValidationFailure when row spacing is zero',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
      ...
      test(
        'should return ValidationFailure when row spacing is too small',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
      ...
      test(
        'should return ValidationFailure when row spacing is too large',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
    ...
    group('Validation - Germination', () {
      test(
        'should return ValidationFailure when germination is zero',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
      ...
      test(
        'should return ValidationFailure when germination is above 100',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
      ...
      test(
        'should return ValidationFailure when germination is too low',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
    ...
    group('Validation - Disc Holes', () {
      ...
      test(
        'should return ValidationFailure when disc holes is invalid',
        () async {
          // Arrange
          const params = CalculatePlanterSetupParams(
            cropType: 'Soja',
          ...
          result.fold((failure) {
            ...
            expect(
              failure.message,
              contains('Número de furos do disco inválido'),
            );
    ...
    group('Edge Cases', () {
      ...
      test('should handle decimal precision correctly', () async {
        ...
        result.fold((failure) => fail('Should not return failure'), (
          ...
          expect(
            calculation.seedsPerMeter.toString().split('.').length,
            lessThanOrEqualTo(2),
          );
          expect(
            calculation.seedWeight.toString().split('.').length,
            lessThanOrEqualTo(2),
          );
        });
      });
    ...
    group('Timestamp', () {
      test('should set calculatedAt timestamp', () async {
        ...
        result.fold((failure) => fail('Should not return failure'), (
          ...
          expect(
            calculation.calculatedAt.isAfter(
              beforeCalculation.subtract(const Duration(seconds: 1)),
            ),
            true,
          );
          expect(
            calculation.calculatedAt.isBefore(
