...
/// Dialog showing all achievements organized by category
class AchievementsDialog extends ConsumerWidget {
  ...
  Widget _buildHeader(BuildContext context, AchievementStats stats) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  Widget _buildSummary(AchievementStats stats) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildStat('Restantes', stats.remaining.toString(), Icons.lock_outline),
        ],
      ),
    );
  }
  ...
  Widget _buildTabs(BuildContext context, List<Achievement> achievements) {
          ...
          TabBar(
            ...
            tabs: AchievementCategory.values.map((cat) {
              final count = achievements
                  .where((a) =>
                      a.definition.category == cat && a.isUnlocked)
                  .length;
              final total = AchievementDefinitions.getByCategory(cat).length;
              return Tab(
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
  ...
  Widget _buildCategoryList(List<Achievement> achievements) {
...
/// Card showing a single achievement
class AchievementCard extends StatelessWidget {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isLocked
              ? Colors.grey[700]!
              : def.rarity.color.withOpacity(0.5),
          width: isLocked ? 1 : 2,
        ),
        boxShadow: !isLocked
            ? [
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: def.rarity.color.withOpacity(isLocked ? 0.3 : 0.8),
                          borderRadius: BorderRadius.circular(10),
                        ),
                        child: Text(
                          def.rarity.label,
                    ...
                    Column(
                      children: [
                        LinearProgressIndicator(
                          ...
                          valueColor: AlwaysStoppedAnimation(def.rarity.color),
                        ),
                        const SizedBox(height: 4),
                        Text(
  ...
  Color _getUnlockedColor(AchievementRarity rarity) {
...
/// Dialog shown when achievement is unlocked
...
class AchievementsUnlockedOverlay extends StatefulWidget {
  final List<AchievementDefinition> achievements;
  final VoidCallback onComplete;

  const AchievementsUnlockedOverlay({
    super.key,
  ...
  @override
  State<AchievementsUnlockedOverlay> createState() =>
      _AchievementsUnlockedOverlayState();
}

class _AchievementsUnlockedOverlayState
    extends State<AchievementsUnlockedOverlay> {
  int _currentIndex = 0;
...
class AchievementsProgressIndicator extends ConsumerWidget {
  const AchievementsProgressIndicator({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final completionPercent = ref.watch(achievementsCompletionPercentProvider);
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            ...
            Text(
              '$unlockedCount/${AchievementDefinitions.totalCount}',
              style: const TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
              ),
