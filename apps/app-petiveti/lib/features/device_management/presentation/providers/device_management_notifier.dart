...
class DeviceManagementState {
  ...
  int get activeMobileDeviceCount => activeDevices
      .where((d) => petivetiDeviceLimitConfig.isMobilePlatform(d.platform))
      .length;
  
  /// Conta dispositivos web ativos
  int get activeWebDeviceCount => activeDevices
      .where((d) => petivetiDeviceLimitConfig.isWebOrDesktopPlatform(d.platform))
      .length;
  ...
  bool get canAddMoreDevices => activeMobileDeviceCount < petivetiDeviceLimitConfig.maxMobileDevices;
  ...
  Color get statusColor {
    ...
    if (activeMobileDeviceCount < petivetiDeviceLimitConfig.maxMobileDevices) return Colors.green;
    return Colors.orange;
  }

  IconData get statusIcon {
    if (!hasDevices) return Icons.devices_other;
    if (activeMobileDeviceCount < petivetiDeviceLimitConfig.maxMobileDevices) return Icons.verified;
    return Icons.warning;
  }

  /// Mostra apenas dispositivos mobile no limite (web não conta)
  String get deviceLimitText => '$activeMobileDeviceCount/${petivetiDeviceLimitConfig.maxMobileDevices} dispositivos móveis';

  bool get isNearDeviceLimit => activeMobileDeviceCount >= (petivetiDeviceLimitConfig.maxMobileDevices - 1);

  bool get hasReachedDeviceLimit => activeMobileDeviceCount >= petivetiDeviceLimitConfig.maxMobileDevices;
...
class DeviceManagementNotifier extends _$DeviceManagementNotifier {
  ...
  Future<DeviceManagementState> build() async {
    _deviceManagementService = ref.watch(petivetiDeviceManagementServiceProvider);
...
// deviceManagementNotifierProvider is the main provider exported by code generation
