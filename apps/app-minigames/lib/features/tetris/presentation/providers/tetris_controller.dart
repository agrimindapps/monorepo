...
class TetrisController extends _$TetrisController {
    ...
    for (int row = 0; row < piece.shape.length; row++) {
      for (int col = 0; col < piece.shape[row].length; col++) {
        if (piece.shape[row][col] == 1) {
          ...
          if (boardY >= 0 &&
              boardY < TetrisState.boardHeight &&
              boardX >= 0 &&
              boardX < TetrisState.boardWidth) {
  ...
  void _moveDown() {
    ...
    if (_canPlace(piece, piece.x, piece.y + 1)) {
      state = state.copyWith(currentPiece: piece.copy()..y = piece.y + 1);
    } else {
      _lockPiece();
    }
  }
  ...
  void moveLeft() {
    if (state.currentPiece == null || state.isGameOver || state.isPaused)
      return;

    final piece = state.currentPiece!;
    if (_canPlace(piece, piece.x - 1, piece.y)) {
      state = state.copyWith(currentPiece: piece.copy()..x = piece.x - 1);
    }
  }
  ...
  void hardDrop() {
    ...
    state = state.copyWith(currentPiece: piece.copy()..y = newY);
    _lockPiece();
  }
  ...
  Future<void> _saveScore() async {
    ...
    try {
      ...
      await scoreActions.saveScore(score);
    } catch (e) {
