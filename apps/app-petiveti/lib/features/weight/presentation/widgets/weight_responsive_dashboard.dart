...
class WeightResponsiveDashboard extends ConsumerWidget {
  ...
  const WeightResponsiveDashboard({super.key, this.selectedAnimalId});
              ...
              children: [
                Expanded(
                  ...
                  child: Column(
                    children: [
                      Expanded(
                        child: WeightChartVisualization(
  ...
  List<_InsightItem> _calculateInsights(List<Weight> weights) {
    ...
    if (change.abs() > 0.1) {
      insights.add(
        _InsightItem(
          ...
          description:
              'Variação de ${change.abs().toStringAsFixed(1)} kg desde o último registro',
    ...
    if (weights.length >= 3) {
      ...
      if (recentWeights.length >= 2) {
        final weeklyChange =
            recentWeights.first.weight - recentWeights.last.weight;
        insights.add(
          _InsightItem(
