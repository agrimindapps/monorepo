...
class DatabaseRepository {
  ...
  Future<List<Map<String, dynamic>>> loadJsonFiles(String table) async {
    List<Map<String, dynamic>> filenames = listaDbJson();
  ...
  Future<List<dynamic>> getIndex(
    ...
    final List<Map<String, dynamic>> data = await loadJsonFiles(table);
  ...
  //   try {
  //     Uint8List decodedBytes = base64.decode(text);
  //     String d = latin1.decode(decodedBytes);
  //     List<String> c1 = d.substring(0, d.length ~/ 2).split('');
  //     List<String> c2 = d.substring(d.length ~/ 2, d.length).split('');
  //     String b = '';
  //     for (int x = 0; x < d.length ~/ 2; x++) {
  //       b += c1[x];
  //       b += c2[x];
  //     }
  //     return latin1.decode(base64.decode(b));
  //   } catch (e) {
  //     // ignore: avoid_print
  //     debugPrint('Conversão com problemas');
  //   } finally {
  //     return '';
  //   }
  // }
