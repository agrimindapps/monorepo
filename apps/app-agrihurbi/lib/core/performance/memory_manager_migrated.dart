...
part 'memory_manager_migrated.g.dart';
...
/// Pure Riverpod Notifier para gerenciamento de memória
///
...
class Memory extends _$Memory {
  ...
  @override
  MemoryState build() {
    final initialState = MemoryState(startTime: DateTime.now());
  ...
  double _estimateMemoryUsage() {
    ...
    final runTimeMinutes = state.startTime != null
        ...
        : 0;
    final timeMB =
        (runTimeMinutes * 0.1).clamp(0.0, 50.0); // Max 50MB por tempo
  ...
  void markCacheAccessed(String name) {
    ...
    if (entry != null) {
      final newManagedCaches =
          Map<String, CacheEntry>.from(state.managedCaches);
