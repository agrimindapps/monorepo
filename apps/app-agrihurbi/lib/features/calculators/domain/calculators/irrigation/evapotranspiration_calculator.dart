...
class EvapotranspirationCalculator extends CalculatorEntity {
  const EvapotranspirationCalculator()
    : super(
        ...
        description:
            'Calcula a evapotranspiração de referência pelo método de Penman-Monteith (FAO-56)',
        ...
        parameters: const [
          ...
          CalculatorParameter(
            ...
            defaultValue: 18.0,
            validationMessage:
                'Temperatura mínima deve estar entre -10°C e 40°C',
          ),
          CalculatorParameter(
            id: 'humidity',
          ...
          CalculatorParameter(
            ...
            defaultValue: 2.0,
            validationMessage:
                'Velocidade do vento deve estar entre 0.1 e 20.0 m/s',
          ),
          CalculatorParameter(
            id: 'solar_radiation',
            name: 'Radiação Solar',
            ...
            defaultValue: 20.0,
            validationMessage:
                'Radiação solar deve estar entre 5 e 35 MJ/m²/dia',
          ),
          CalculatorParameter(
            id: 'altitude',
            name: 'Altitude',
  ...
  CalculationResult calculate(Map<String, dynamic> inputs) {
    try {
      ...
      final double solarRadiation = double.parse(
        inputs['solar_radiation'].toString(),
      );
      final double altitude = double.parse(inputs['altitude'].toString());
      double.parse(inputs['latitude'].toString());
      if (tempMax <= tempMin) {
      ...
      final double atmPressure =
          101.3 * math.pow((293 - 0.0065 * altitude) / 293, 5.26);
      ...
      final double esMin =
          0.6108 * math.exp(17.27 * tempMin / (tempMin + 237.3));
      ...
      final double radiationMmDay =
          solarRadiation * 0.408; // Conversão para mm/dia
  ...
  List<String> _generateRecommendations(
    ...
    if (eto < 2.0) {
      recommendations.add(
        'Baixa demanda evaporativa. Irrigação menos frequente.',
      );
    } else if (eto > 6.0) {
      recommendations.add(
        'Alta demanda evaporativa. Aumente frequência de irrigação.',
      );
    }
    if (humidity < 40) {
      recommendations.add(
        'Baixa umidade relativa. Monitore stress hídrico das plantas.',
      );
    } else if (humidity > 80) {
    ...
    if (windSpeed > 4.0) {
      recommendations.add(
        'Vento forte. Pode aumentar evapotranspiração e deriva na aplicação.',
      );
    }
    if (tempMean > 30) {
      recommendations.add(
        'Temperaturas elevadas. Considere irrigação nos horários mais frescos.',
      );
    }
    if (vpd > 2.0) {
      recommendations.add(
        'Alto déficit de pressão de vapor. Risco de stress hídrico.',
      );
    }
