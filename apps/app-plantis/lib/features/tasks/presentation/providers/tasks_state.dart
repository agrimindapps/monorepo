...
class TasksState extends Equatable {
  ...
  TasksState copyWith({
    ...
    return TasksState(
      ...
      individualTaskOperations: individualTaskOperations ?? this.individualTaskOperations,
      activeOperations: activeOperations ?? this.activeOperations,
      currentOperationMessage: clearOperationMessage ? null : currentOperationMessage ?? this.currentOperationMessage,
    );
  }
  ...
  bool isTaskOperationLoading(String taskId) => individualTaskOperations[taskId] ?? false;
  bool get hasActiveOperations => activeOperations.isNotEmpty;
  bool isOperationActive(TaskLoadingOperation operation) => activeOperations.contains(operation);
  ...
  int get completedTasks => allTasks
      .where((t) => t.status == task_entity.TaskStatus.completed)
      .length;

  int get pendingTasks => allTasks
  ...
  List<task_entity.Task> get overdueTasksList => allTasks
      .where(
        (t) => t.isOverdue && t.status == task_entity.TaskStatus.pending,
      )
      .toList();
  ...
  /// Get today's tasks
  List<task_entity.Task> get todayTasksList => allTasks
      .where(
        (t) => t.isDueToday && t.status == task_entity.TaskStatus.pending,
      )
      .toList();
  ...
  /// Get completed tasks
  List<task_entity.Task> get completedTasksList => allTasks
      .where((t) => t.status == task_entity.TaskStatus.completed)
      .toList();
