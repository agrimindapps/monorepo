...
class CompostCalculator extends CalculatorEntity {
  const CompostCalculator()
    : super(
        ...
        description:
            'Calcula proporções ideais de materiais para compostagem, tempo de decomposição e qualidade do composto final',
        ...
        parameters: const [
          ...
          CalculatorParameter(
            ...
            description:
                'Quantidade disponível do material rico em carbono (kg)',
          ...
          CalculatorParameter(
            ...
            description:
                'Quantidade disponível do material rico em nitrogênio (kg)',
            type: ParameterType.decimal,
            unit: ParameterUnit.quilograma,
            minValue: 5.0,
            maxValue: 5000.0,
          ...
          CalculatorParameter(
            ...
            description:
                'Relação carbono/nitrogênio desejada para a compostagem',
            type: ParameterType.decimal,
  ...
  CalculationResult calculate(Map<String, dynamic> inputs) {
    try {
      ...
      final double brownAmount = double.parse(
        inputs['brown_amount'].toString(),
      );
      final String greenMaterialType = inputs['green_material_type'].toString();
      final double greenAmount = double.parse(
        inputs['green_amount'].toString(),
      );
      final double targetCNRatio = double.parse(
        inputs['target_cn_ratio'].toString(),
      );
      final double moistureContent = double.parse(
        inputs['moisture_content'].toString(),
      );
      final String pileSize = inputs['pile_size'].toString();
      final double ambientTemperature = double.parse(
        inputs['ambient_temperature'].toString(),
      );
      final String turningFrequency = inputs['turning_frequency'].toString();
      final double desiredFinalAmount = double.parse(
        inputs['desired_final_amount'].toString(),
      );
      final Map<String, dynamic> brownData = _getMaterialData(
        brownMaterialType,
        'brown',
      );
      final Map<String, dynamic> greenData = _getMaterialData(
        greenMaterialType,
        'green',
      );
      ...
      final Map<String, dynamic> optimalProportions =
          _calculateOptimalProportions(
            brownData,
            greenData,
            targetCNRatio,
            desiredFinalAmount,
      ...
      final Map<String, dynamic> pileDimensions = _calculatePileDimensions(
        optimalProportions['total_material'] as double,
        pileSize,
        compostType,
      );
      final List<Map<String, dynamic>> managementSchedule =
          _generateManagementSchedule(
            timeEstimate['total_time_days'] as int,
            turningFrequency,
            compostType,
          );
      ...
      return CalculationResult(
        ...
        values: [
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              optimalProportions['resulting_cn'] as double,
              1,
            ),
            unit: ':1',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              pileDimensions['total_volume'] as double,
              1,
            ),
            unit: 'm³',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              (timeEstimate['total_time_days'] as int).toDouble(),
              0,
            ),
            unit: 'dias',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              (timeEstimate['thermophilic_days'] as int).toDouble(),
              0,
            ),
            unit: 'dias',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              optimalProportions['final_yield'] as double,
              0,
            ),
            unit: 'kg',
          ...
          CalculationResultValue(
            label: 'N Total Final',
            value: CalculatorMath.roundTo(
              finalNutrients['total_n'] as double,
              2,
            ),
            unit: '%',
          ...
          CalculationResultValue(
            label: 'P Total Final',
            value: CalculatorMath.roundTo(
              finalNutrients['total_p'] as double,
              2,
            ),
            unit: '%',
          ...
          CalculationResultValue(
            label: 'K Total Final',
            value: CalculatorMath.roundTo(
              finalNutrients['total_k'] as double,
              2,
            ),
            unit: '%',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              finalNutrients['organic_matter'] as double,
              1,
            ),
            unit: '%',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              costAnalysis['total_cost'] as double,
              0,
            ),
            unit: 'R\$',
          ...
          CalculationResultValue(
            ...
            value: CalculatorMath.roundTo(
              costAnalysis['cost_per_kg'] as double,
              2,
            ),
            unit: 'R\$/kg',
  ...
  Map<String, dynamic> _calculateCNRatio(
    ...
    final double brownCarbon =
        brownAmount * (brownData['carbon_content'] as double) / 100;
    final double brownNitrogen =
        brownAmount * (brownData['nitrogen_content'] as double) / 100;
    final double greenCarbon =
        greenAmount * (greenData['carbon_content'] as double) / 100;
    final double greenNitrogen =
    ...
    final double currentCNRatio =
        totalNitrogen > 0 ? totalCarbon / totalNitrogen : 0;
  ...
  Map<String, dynamic> _calculateOptimalProportions(
    ...
    final double ratio =
        (targetCN * greenN - greenC) / (brownC - targetCN * brownN);
    final double totalMaterialNeeded =
        desiredFinalAmount / 0.4; // Assumindo 40% de rendimento
    ...
    final double resultingC =
        (brownNeeded * brownC + greenNeeded * greenC) / 100;
    final double resultingN =
        (brownNeeded * brownN + greenNeeded * greenN) / 100;
  ...
  Map<String, dynamic> _estimateCompostingTime(
    ...
    int maturationDays =
        totalTime - thermophilicDays - mesophilicDays; // Resto em maturação
  ...
  Map<String, dynamic> _calculateFinalNutrients(
    ...
    final double initialN =
        (brownAmount * (brownData['nitrogen_content'] as double) +
            greenAmount * (greenData['nitrogen_content'] as double)) /
        100;
    final double initialP =
        (greenAmount * ((greenData['p_content'] as double?) ?? 0.5)) / 100;
    final double initialK =
        (greenAmount * ((greenData['k_content'] as double?) ?? 1.0)) / 100;
  ...
  Map<String, dynamic> _calculateCostAnalysis(
    ...
    final double brownCost =
        (proportions['brown_needed'] as double) *
        (materialCosts[brownMaterial] ?? 0.10);
    final double greenCost =
        (proportions['green_needed'] as double) *
  ...
  List<String> _generateRecommendations(
    ...
    if (cnRatio > 35) {
      recommendations.add(
        'Relação C/N alta: adicionar mais material verde (rico em N).',
      );
    } else if (cnRatio < 25) {
      recommendations.add(
        'Relação C/N baixa: adicionar mais material marrom (rico em C).',
      );
    }
    if (moisture < 45) {
      recommendations.add(
        'Umidade baixa: adicionar água durante o reviramento.',
      );
    } else if (moisture > 65) {
      recommendations.add(
        'Umidade alta: adicionar material seco e aumentar aeração.',
      );
    }
    if (temperature < 15) {
      recommendations.add(
        'Temperatura baixa: considerar cobertura da leira e local protegido.',
      );
    ...
    switch (compostType) {
      case 'Vermicompostagem':
        recommendations.add(
          'Vermicompostagem: manter temperatura abaixo de 35°C.',
        );
        recommendations.add(
          'Evitar materiais ácidos que prejudicam as minhocas.',
        );
        break;
      case 'Compostagem Termofílica':
        recommendations.add(
          'Termofílica: monitorar temperatura diariamente na fase ativa.',
        );
        break;
      case 'Bokashi':
        recommendations.add(
          'Bokashi: manter sistema anaeróbico durante fermentação.',
        );
        break;
    }
    recommendations.add(
      'Triturar materiais grandes para acelerar decomposição.',
    );
    ...
    recommendations.add(
      'Composto pronto: cor escura, cheiro de terra, não esquenta.',
    );
