...
class AuthRateLimiter {
  ...
  Future<bool> canAttemptLogin() async {
    ...
    if (lockoutEndTimeStr != null) {
      ...
      if (now < lockoutEndTime) {
        ...
        if (kDebugMode) {
          debugPrint(
            '🔒 INTERNO: Usuário bloqueado - restam $remainingMin minutos',
          );
        }
  ...
  Future<void> recordFailedAttempt() async {
    ...
    final lastAttemptTimeStr = await _secureStorage.read(
      key: _lastAttemptTimeKey,
    );
    ...
    if (attemptCount >= _maxAttempts) {
      ...
      await _secureStorage.write(
        key: _lockoutEndTimeKey,
        value: lockoutEndTime.toString(),
      );

      if (kDebugMode) {
        debugPrint(
          '🔒 INTERNO: Rate limiting ativado - $attemptCount tentativas em $_attemptWindowMinutes min',
        );
