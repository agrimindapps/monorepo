...
class UpdatePragaUseCase implements UseCase<Praga, Praga> {
  ...
  UpdatePragaUseCase(this.repository);

  @override
  Future<Either<Failure, Praga>> call(Praga praga) async {
    // Validation
    if (praga.id.trim().isEmpty) {
      return Left(ValidationFailure('ID é obrigatório para atualização'));
    }
    ...
    return repository.updatePraga(praga);
  }
}