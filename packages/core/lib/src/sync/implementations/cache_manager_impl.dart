...
class CacheManagerImpl implements ICacheManager {
  ...
  Future<Either<Failure, void>> initialize() async {
    if (_isDisposed) {
      return Left(CacheFailure('Cache manager has been disposed'));
    }
  ...
  Future<Either<Failure, void>> put(
    ...
    try {
      ...
      _emitEvent(CacheEvent(
        type: _cache.containsKey(key) ? CacheEventType.entryUpdated : CacheEventType.entryAdded,
        key: key,
  ...
  Future<Either<Failure, T?>> get<T>(String key) async {
    ...
    try {
      ...
      if (entry.isExpired) {
        ...
        _emitEvent(CacheEvent(
          type: CacheEventType.entryExpired,
          key: key,
        ));
  ...
  Future<Either<Failure, void>> remove(String key) async {
    ...
    if (_cache.remove(key) != null) {
      _emitEvent(CacheEvent(
        type: CacheEventType.entryRemoved,
        key: key,
      ));
  ...
  Future<Either<Failure, void>> clear() async {
    ...
    developer.log('Cache cleared: $count entries removed', name: 'CacheManager');
    return const Right(null);
  }
  ...
  Future<Either<Failure, void>> clearExpired() async {
    ...
    developer.log('Expired cache cleanup: ${expiredKeys.length} entries removed', name: 'CacheManager');
    return const Right(null);
  }
  ...
  Future<CacheStatistics> getStatistics() async {
    ...
    final memoryUsageBytes = _cache.values.fold<int>(0, (sum, entry) => sum + entry.sizeBytes);
  ...
  Future<CacheHealthCheck> checkHealth() async {
    ...
    final memoryUsage = _cache.values.fold<int>(0, (sum, e) => sum + e.sizeBytes);
    if (memoryUsage > _cleanupStrategy.maxMemoryBytes) {
      issues.add('Memory usage exceeds limit: ${memoryUsage / (1024 * 1024)}MB');
    }
    ...
    metrics['hit_rate'] = (_hitCount + _missCount) > 0 ? (_hitCount / (_hitCount + _missCount)) * 100 : 0;
  ...
  Future<void> _evictByMemory(int bytesToEvict) async {
    ...
    developer.log('Evicted $evictedCount entries (${evictedBytes}b) by memory', name: 'CacheManager');
  }
