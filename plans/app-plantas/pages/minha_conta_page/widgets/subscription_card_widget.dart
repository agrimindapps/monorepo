...
class SubscriptionCardWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetX<SubscriptionService>(
      init: SubscriptionService.instance,
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: PlantasColors.subtitleColor.withValues(alpha: 0.1),
              ...
              child: Center(
                child: CircularProgressIndicator(
                  strokeWidth: 2,
                  valueColor:
                      AlwaysStoppedAnimation<Color>(PlantasColors.primaryColor),
                ),
              ),
            ),
  ...
  Widget _buildSubscriptionInfo(SubscriptionModel subscription) {
    ...
    switch (subscription.status) {
      case SubscriptionStatus.active:
        if (subscription.diasRestantes > 0) {
          texto =
              'Sua assinatura está ativa e renova em ${subscription.diasRestantes} dias';
          cor = Colors.green;
        } else {
      ...
      case SubscriptionStatus.canceled:
        texto =
            'Assinatura cancelada - acesso até ${_formatarData(subscription.terminaEm)}';
        cor = Colors.orange;
        break;
      case SubscriptionStatus.expired:
        texto =
            'Sua assinatura expirou em ${_formatarData(subscription.terminaEm)}';
        cor = Colors.red;
        break;
      default:
  ...
  Widget _buildProgressBar(SubscriptionModel subscription) {
    ...
    final totalDias =
        subscription.terminaEm!.difference(subscription.inicioEm!).inDays;
    ...
    return Column(
      ...
      children: [
        ...
        LinearProgressIndicator(
          value: progresso,
          backgroundColor: PlantasColors.borderColor,
  ...
  void _handleUpgradeAction() {
    ...
    SubscriptionService.instance.navegarParaPlanos();
  }

  void _handleMenuAction(String action, SubscriptionModel subscription) {
    switch (action) {
      case 'gerenciar':
        SubscriptionService.instance.navegarParaGerenciarAssinatura();
        break;
      case 'restaurar':
        SubscriptionService.instance.restoreSubscription();
        break;
      case 'cancelar':
  ...
  Future<void> _handleCancelSubscription() async {
    final confirmar =
        await SubscriptionService.instance.mostrarDialogoCancelamento();
    if (confirmar) {
