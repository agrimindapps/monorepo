...
class LegacySyncBridge {
  ...
  Future<Either<Failure, ServiceSyncResult>> _syncWithNewArchitecture(
    String appName,
  ) async {
    try {
      developer.log(
      ...
      return result.fold((failure) => Left(failure), (syncResult) {
        developer.log(
  ...
  Future<Either<Failure, dynamic>> _syncWithLegacyArchitecture(
    String appName,
  ) async {
    try {
      developer.log(
      ...
      return const Right('Legacy sync completed successfully');
    } catch (e) {
  ...
  Map<String, Map<String, dynamic>> getAllAppsStatus() {
    ...
    for (final appName in ['gasometer', 'plantis', 'receituagro', 'petiveti']) {
      final isUsingNew =
          flags.isEnabledForApp(appName) && flags.useNewSyncOrchestrator;
  ...
  Future<Either<Failure, void>> migrateAppToNewArchitecture(
    String appName,
  ) async {
    try {
      developer.log(
  ...
  Future<void> dispose() async {
    developer.log('Disposing Legacy Sync Bridge', name: 'LegacySyncBridge');
