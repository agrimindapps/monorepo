...
/// Migrado para Riverpod 3.0
extension WidgetRefExtensions on WidgetRef {
  /// Invalida múltiplos providers de uma vez
  void invalidateMultiple(List<ProviderOrFamily> providers) {
...
// StateController extensions foram removidas - deprecado no Riverpod 3.0
// Use Notifier.state diretamente para atualizar estado
...
/// Widget que consome um AsyncNotifierProvider e automaticamente mostra loading/error
/// Migrado para Riverpod 3.0 - usa typed provider diretamente
class AsyncProviderConsumerWidget<T> extends ConsumerWidget {
  const AsyncProviderConsumerWidget({
    super.key,
...
/// Widget simplificado para consumir FutureProvider
class FutureProviderConsumerWidget<T> extends ConsumerWidget {
  const FutureProviderConsumerWidget({
    super.key,
    required this.provider,
    required this.builder,
    this.loadingWidget,
...
// createStateNotifierProvider foi removido - deprecado no Riverpod 3.0
// Use NotifierProvider diretamente:
// final myProvider = NotifierProvider<MyNotifier, MyState>(MyNotifier.new);

/// Helper para criar NotifierProvider com menos boilerplate - Riverpod 3.0
NotifierProvider<TNotifier, TState>
    createNotifierProvider<TNotifier extends Notifier<TState>, TState>(
  TNotifier Function() create, {
  String? name,
}) {
  return NotifierProvider<TNotifier, TState>(create, name: name);
}
