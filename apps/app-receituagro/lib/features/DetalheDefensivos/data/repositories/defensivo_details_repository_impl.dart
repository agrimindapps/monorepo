...
class DefensivoDetailsRepositoryImpl implements IDefensivoDetailsRepository {
  ...
  DefensivoDetailsRepositoryImpl({
    ...
    IDiagnosticosRepository? diagnosticosRepository,
       ...
       _diagnosticosRepository =
           diagnosticosRepository ?? sl<IDiagnosticosRepository>();

  @override
  Future<Either<Failure, DefensivoDetailsEntity?>> getDefensivoByName(
    String name,
  ) async {
    try {
      ...
      final defensivos = result.data!.where(
        (d) => d.nomeComum == name || d.nomeTecnico == name,
      );
  ...
  @override
  Future<Either<Failure, List<DiagnosticoEntity>>> getDiagnosticosByDefensivo(
    String defensivoId,
  ) async {
    try {
      ...
      return result.fold((failure) => Left(failure), (diagnosticos) {
        final entities = DiagnosticoMapper.fromDiagnosticosEntityList(
          diagnosticos,
        );
      ...
      return Left(
        ServerFailure('Erro ao buscar diagnósticos: ${e.toString()}'),
      );
    }
  }
  ...
  Future<Either<Failure, bool>> toggleFavorite(
    ...
    try {
      final isCurrentlyFavorited = await _favoritosRepository.isFavorito(
        'defensivos',
        defensivoId,
      );
      ...
      if (isCurrentlyFavorited) {
        success = await _favoritosRepository.removeFavorito(
          'defensivos',
          defensivoId,
        );
      } else {
        success = await _favoritosRepository.addFavorito(
          'defensivos',
          defensivoId,
          defensivoData,
        );
